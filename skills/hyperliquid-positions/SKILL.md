...
curl -sS -X POST $BASE/info -H 'Content-Type: application/json' -d '{"type":"meta"}' \
 | jq --arg c ETH '(.universe[] | select(.name==$c)) as $u | {name:$u.name, szDecimals:$u.szDecimals, maxLeverage:$u.maxLeverage, marginTableId:$u.marginTableId,
      table: (.marginTables[] | select(.[0]==$u.marginTableId) | .[1])}'
```

`marginTables` is a list of `[id, {description, marginTiers: [{lowerBound, maxLeverage}, ...]}]`; the tier whose `lowerBound` (position notional in USD) is the largest one at or below your notional applies. Ids below 50 mean a single tier whose max leverage equals the id. Testnet tiers are far tighter than mainnet (BTC drops from 40x above only 10k notional on testnet), which is one reason testnet rehearsals differ from mainnet.
...
Do this **before** the entry the ticket refers to. Changing leverage on a market with an open position changes its margin requirement immediately.
...
print(next(p["position"]["leverage"] for p in info.user_state(ACCOUNT)["assetPositions"] if p["position"]["coin"]=="ETH"))
```
...
A position is protected when a **reduce-only trigger order** on the opposite side, sized at least to the position, is resting on the exchange. Verify with `frontendOpenOrders`, which exposes trigger fields:
...
`side` is `B` (buy) or `A` (sell). A long's stop is an `A` trigger below mark with `reduceOnly: true`; a short's is a `B` trigger above mark. Missing or undersized: report **unprotected** to the Desk Lead (`desk-incident-response` playbook D).
...
- Changing leverage with an open position without telling the user; margin used and liquidation price move at once.
...
- Leaving TP/SL orphans after a close; they can open a new position later.
