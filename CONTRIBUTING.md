...
+ `scripts/test_check_manifests.py` - negative fixtures that prove the manifest check fails on a version mismatch, a missing component path, invalid JSON, inventory drift, a documented install command that no longer resolves, and a skills.sh listing or `skills add` argument that names a slug this repository does not ship.
