...
- Prefer `positionTpsl` grouping for stops that should follow the position and `normalTpsl` when the stop belongs to a specific entry; the ticket says which, the skill explains both.
...
sent: order ETH-PERP buy 0.51 @ 3000.0 Gtc reduceOnly=false cloid=0x9f3e...c1a2 grouping=positionTpsl (+ sl trigger 2900 market sell 0.51 reduceOnly)
response: statuses[0] resting oid 1839201122; statuses[1] resting oid 1839201123 (waitingForTrigger)
...
- "Move the SOL stop to 150." — modify the trigger order or cancel and re-place per the skill, after approval.
