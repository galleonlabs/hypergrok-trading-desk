...
7. **One action.** Entry plus its stop and take-profit go in one `order` action with `grouping` set (`normalTpsl` for a stop tied to this entry's size, `positionTpsl` for a stop that follows the whole position). Anything else in the ticket that is a different action type (leverage change, cancel) is a separate, separately approved step.
...
- **Modify:** Hyperliquid's `modify`/`batchModify` replaces an order in place, or cancel and re-place. Never leave the position unprotected between the two; place the new stop before cancelling the old one when both can coexist.
