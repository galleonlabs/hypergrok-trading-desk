...
Python: `info.user_state(ADDR)`. `szi` is signed size; `leverage` is `{type: cross|isolated, value, rawUsd?}`; `crossMarginSummary` mirrors `marginSummary` for the cross portion. Margin ratio for a book check: `crossMaintenanceMarginUsed / marginSummary.accountValue`.

Under the account's abstraction mode (`{"type":"userAbstraction","user":ADDR}` returns `default`, `unifiedAccount`, `portfolioMargin`...), USDC may live in the spot state; check both when equity looks wrong.
...
`crossed: true` means taker; `fee` includes any builder fee and is negative for a rebate; `dir` reads like `Open Long`, `Close Short`; `startPosition` is the size before the fill; `closedPnl` is realised on that fill. `userFills` returns the most recent 2000; `userFillsByTime` returns up to 2000 per call from the last 10,000 (paginate with `startTime` = last `time` + 1). Python: `info.user_fills(ADDR)`, `info.user_fills_by_time(ADDR, start_ms, end_ms)`.
