...
```bash
hl '{"type":"meta"}' | jq --arg c BTC '(.universe[] | select(.name==$c)) as $u
  | {name:$u.name, maxLeverage:$u.maxLeverage, marginTableId:$u.marginTableId,
     tiers: (.marginTables[] | select(.[0]==$u.marginTableId) | .[1].marginTiers)}'
```
...
- Pulling candles in one call for long histories; the cap is 5000 per request.
