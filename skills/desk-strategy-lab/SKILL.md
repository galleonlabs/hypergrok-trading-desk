...
- data: 1h candles (close), hourly funding from fundingHistory
- entry: when funding rate > +0.005%/h for 8 consecutive hours, sell at next open
...
Candles via `candleSnapshot` (`hyperliquid-market-data`; note the per-request cap on candle count and paginate by time), funding via `fundingHistory` (paginate; oldest first). Save to CSV under `data/` with the request recorded in `data.md`. Check gaps and duplicated timestamps before using the file. Note that Hyperliquid's candle history begins when the market was listed; older markets have more history.
