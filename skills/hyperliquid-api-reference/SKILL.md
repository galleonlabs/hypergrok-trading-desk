...
- `POST /info` body `{"type": "<request>", ...}`, unsigned; response is the bare payload. Malformed body: HTTP 422.
...
| `modify` / `batchModify` | `oid (or cloid), order:{...}` / `modifies:[{oid, order}]` | L1 | yes |
...
| `twapOrder` / `twapCancel` | `twap:{a,b,s,r,m,t}` / `a, t (twapId)` | L1 | yes, on request |
| `noop` | | L1 | rarely |
