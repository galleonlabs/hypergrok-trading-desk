...
- Account: `clearinghouseState` for equity (`marginSummary.accountValue`), free margin (`withdrawable` or `accountValue - totalMarginUsed`), positions (`assetPositions[].position`: `coin`, `szi`, `entryPx`, `leverage`, `liquidationPx`, `marginUsed`, `unrealizedPnl`) and open orders via `openOrders` / `frontendOpenOrders`. Skill: `hyperliquid-account`.
...
- equity, free margin, `crossMaintenanceMarginUsed`, margin ratio (maintenance margin used / account value), and the distance from mark to `liquidationPx` per position in percent
...
- protection: for each position, is there a reduce-only stop resting on the exchange (trigger order, `reduceOnly: true`, correct side, size at least the position size)? If not: **unprotected**, flagged as an incident to the Desk Lead
