...
- Partial fill on an IOC: the unfilled remainder is gone; the position is smaller than the ticket. Report the actual size; the stop ticket must match the actual size (positionTpsl handles this automatically; normalTpsl children keep the original size and may need adjusting).
...
2. Protective stop ticket (trigger, reduce-only, correct side, at least the position size, `positionTpsl`) through Risk PASS and user approval.
...
1. The user revokes the API wallet immediately in the Hyperliquid app (Settings, API) - the desk cannot do this for them and must not delay them.
