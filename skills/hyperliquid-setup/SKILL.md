...
An **API wallet** (Hyperliquid also calls it an agent wallet) is a separate key the user authorises to sign trading actions for their account. It can place and cancel orders and change leverage; it cannot withdraw, transfer or bridge. That is the only kind of key that ever reaches the desk computer.
...
1. Open the app for the chosen network (`https://app.hyperliquid.xyz` or `https://app.hyperliquid-testnet.xyz`), connect the main wallet, open **More, API** (the `/API` page).
...
3. On testnet, fund the account from the faucet (`/drip`) if it has no mock USDC. Testnet USDC is not real.
...
agents = info.extra_agents(account)                     # [{"address","name","validUntil"}, ...]
approved = any(a["address"].lower() == agent_addr.lower() for a in agents)
...
Ready means: `role=agent`, `approved_for_account=True`, `validUntil` in the future, and `account_value` is what the user expects on this network. Anything else: stop and report which line disagrees.
